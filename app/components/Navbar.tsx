...
const Navbar = () => {
    ...
    return (
        <nav
            className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 font-[family-name:var(--font-jost)] bg-white py-4 border-b border-slate-100/50 shadow-sm`}
        >
            <div className="max-w-[1400px] mx-auto px-6 flex justify-between items-center">
                        ...
                        <div key={link.name} className="relative group/item">
                            ...
                            {link.dropdown && (
                                <div className="absolute top-full -left-4 pt-4 opacity-0 invisible group-hover/item:opacity-100 group-hover/item:visible transition-all duration-300 translate-y-2 group-hover/item:translate-y-0">
                                    <div className="bg-white rounded-2xl shadow-2xl border border-green-50 p-4 min-w-[200px]">
                ...
                <div className="hidden lg:flex items-center gap-6">
                    <div className="relative" ref={accountMenuRef}>
                        <button 
                            ...
                            className="w-11 h-11 rounded-full border border-slate-100 text-slate-700 flex items-center justify-center transition-all hover:bg-slate-50 hover:border-green-600 hover:text-green-600"
                        >
                            <User size={20} />
                        </button>
                        ...
                        {/* Account Dropdown */}
                        {showAccountMenu && (
                            <div className="absolute top-full right-0 mt-4 bg-white rounded-2xl shadow-2xl border border-slate-50 p-3 min-w-[180px] animate-in fade-in slide-in-from-top-2 duration-300">
                                <Link 
                                    href="/login" 
                                    className="flex items-center gap-3 px-4 py-3 text-[15px] font-semibold text-slate-700 hover:text-green-600 hover:bg-green-50 rounded-xl transition-all"
                    ...
                    <button className="w-11 h-11 rounded-full border border-slate-100 text-slate-700 flex items-center justify-center transition-all hover:bg-slate-50 hover:border-green-600 hover:text-green-600 relative">
                        <ShoppingBag size={20} />
                ...
                <button
                    ...
                    className="lg:hidden p-2 rounded-xl text-slate-900 transition-colors hover:bg-slate-50"
                >
                ...
                <div className="p-6 flex flex-col h-full">
                    <div className="flex justify-between items-center mb-12">
                        <Link href="/" onClick={() => setIsOpen(false)} className="flex items-center gap-2">
                            <div className="flex flex-col -space-y-1">
                                <span className="text-xl font-black text-slate-900 tracking-tighter uppercase leading-none">ALMA</span>
                                <span className="text-[8px] font-bold text-green-600 uppercase tracking-[0.2em]">Eco-Intelligence</span>
                            </div>
                        </Link>
